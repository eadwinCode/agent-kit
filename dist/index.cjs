...
var AgentResult = class {
  constructor(agentName, output, toolCalls, createdAt, prompt, history, raw, id) {
    ...
    // The id captured when #checksum was memoized. `result.id` is assigned AFTER
    // construction (network.run sets the durable agentMessageId, standalone
    // agent.run its streaming messageId) — if the checksum was read before that,
    // the memo must be invalidated so later reads reflect the stable id.
    __privateAdd(this, _checksumId);
  }
  /**
   ...
   *
   * It is generated by taking a checksum of the message output, tool calls,
   * and the result's (durably minted) id. This allows you to dedupe items
   * when saving conversation history.
   *
   * `createdAt` is deliberately NOT part of the checksum: it is stamped with
   * `new Date()` in code that Inngest re-runs on every step-boundary replay
   * (memoized steps return instantly, the surrounding JS re-executes), so a
   * wall-clock component made the SAME logical result hash differently
   * between the incremental history append and the end-of-run backstop —
   * history adapters that deduped by checksum then persisted duplicate
   * messages. Output/toolCalls come from memoized step results and the id
   * from a durable id-generation step, so this input is replay-stable.
   */
  get checksum() {
    var _a;
    if (__privateGet(this, _checksum) === void 0 || __privateGet(this, _checksumId) !== this.id) {
      const input = JSON.stringify(this.output.concat(this.toolCalls)) + ((_a = this.id) != null ? _a : "");
      __privateSet(this, _checksum, import_xxhashjs.default.h64(input, 0).toString());
      __privateSet(this, _checksumId, this.id);
    }
    return __privateGet(this, _checksum);
  }
};
_checksum = new WeakMap();
_checksumId = new WeakMap();
...
var StreamingContext = class _StreamingContext {
  constructor(config) {
    ...
    this.streamReasoning = (_d = config.streamReasoning) != null ? _d : false;
  }
  /**
  ...
  static fromNetworkState(networkState, config) {
    ...
    return new _StreamingContext({
      ...
      streamReasoning: (_c = config.streamReasoning) != null ? _c : false
    });
  }
  /**
...
var NetworkRun = class extends Network {
  ...
  async execute(...[input, overrides]) {
    ...
    const stopWhen = (_b = overrides == null ? void 0 : overrides.stopWhen) != null ? _b : this.stopWhen;
    let stopInfo;
    const parallelAgents = (_d = (_c = overrides == null ? void 0 : overrides.parallelAgents) != null ? _c : this.parallelAgents) != null ? _d : false;
    ...
    if ((_e = this.history) == null ? void 0 : _e.appendUserMessage) {
      let userMessage;
    ...
    if (streamingPublish) {
      streamingContext = StreamingContext.fromNetworkState(this.state, {
        ...
        scope: "network",
        simulateChunking: (_f = overrides == null ? void 0 : overrides.streaming) == null ? void 0 : _f.simulateChunking,
        chunkSize: (_g = overrides == null ? void 0 : overrides.streaming) == null ? void 0 : _g.chunkSize,
        maxChunksPerMessage: (_h = overrides == null ? void 0 : overrides.streaming) == null ? void 0 : _h.maxChunksPerMessage,
        streamReasoning: (_i = overrides == null ? void 0 : overrides.streaming) == null ? void 0 : _i.streamReasoning
      });
    ...
    try {
      ...
      while (this._stack.length > 0 && (this.maxIter === 0 || this._counter < this.maxIter)) {
        ...
        const batchLimit = parallelAgents ? this.maxIter === 0 ? this._stack.length : Math.min(this._stack.length, this.maxIter - this._counter) : Math.min(1, this._stack.length);
        const batch = [];
        while (batch.length < batchLimit && this._stack.length > 0) {
          const agentName = this._stack.shift();
          const next3 = this._agents.get(agentName);
          if (!next3) {
            break;
          }
          batch.push(next3);
        ...
        const runOne = (p) => p.agent.run(inputContent, {
          network: this,
        ...
        if (batch.length === 1) {
          ...
          for (const [i, p] of prepared.entries()) {
            ...
            if (s.status === "rejected") {
              firstError != null ? firstError : firstError = s.reason;
              continue;
            }
            await this.finalizeAgentCall(p, s.value, inputContent);
          }
          if (firstError !== void 0) {
            throw firstError instanceof Error ? firstError : new Error(
   ...
   */
  async finalizeAgentCall(prepared, call, inputContent) {
...
var Agent = class _Agent {
  ...
  async performInference(p, prompt, history, network, streamingContext, step, signal) {
    var _a;
    ...
    if (streamingContext == null ? void 0 : streamingContext.streamReasoning) {
   ...
   */
  async invokeTools(msgs, network, streamingContext, step, signal) {
    const planned = [];
    ...
    for (const segment of segments) {
      if (signal == null ? void 0 : signal.aborted) {
        for (const p of segment) {
          results[p.pos] = {
            error: import_internals.errors.serializeError(new Error("Tool execution cancelled"))
          };
        }
        continue;
      }
   ...
   */
  async streamToolCallInput(streamingContext, step, tool, index) {
    var _a, _b;
    if (!streamingContext) {
      return;
    }
   ...
   */
  async streamToolCallOutput(streamingContext, step, tool, index, result) {
    if (!streamingContext) {
      return;
    }
    const stepTools = step || await getStepTools();
   ...
   */
  async runToolHandler(toolDef, call, network, step, durableStep, durableIndex) {
    ...
    const index = durableIndex != null ? durableIndex : network.state.nextDurableToolCallIndex();
   ...
   * model-call order.
   */
  applyStatePatch(network, statePatch) {
    if (statePatch !== void 0) {
