...
### Application-defined ownership

```typescript
function ScopedApp({ userId, sessionToken, authorizedChannel }) {
  const scopedTransport = useMemo(() =>
...
### Authorized collaborative channel

```typescript
function CollaborativeWorkspace() {
  // Issued by the server after it authenticates the user and checks access.
  const { userId, channelKey, accessToken } = useChatGrant();
  return (
    <AgentProvider
    ...
    >
      <CollaborativeChat />      {/* Shared conversation */}
...
// The server grant authorizes the channel; its string is only a routing key.
```
...
function IsolatedFeatures() {
  return (
    <div>
      ...
      <AgentProvider
        userId="user-123"
        channelKey="admin-connection" // Separate connection; authorization is server-side
...
```typescript
// Fetch a short-lived grant from an authenticated server endpoint.
function SecureChannelProvider({ children }) {
  const { userId, channelKey, accessToken } = useChatGrant();
...
`channelKey` selects a stream; it is not an authorization boundary. Never
construct a supposedly secure channel solely from client-controlled IDs, and
never trust client-supplied role or ownership headers. The server must verify
the grant when issuing realtime tokens and on every HTTP operation.
...
- **Treat channelKey as authorization** - it is a routing key, not a grant
