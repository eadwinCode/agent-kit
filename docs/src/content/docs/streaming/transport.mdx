  ...
  return (
    <AgentProvider
      ...
      transport={{
        ...
        // Add only non-authoritative application metadata here. The server
        // derives identity and authorization scope from the authenticated
        // session; request-body identifiers are never trusted.
        body: () => ({ clientVersion: "web-2" }),
      }}
    >
      <ChatApp />
    </AgentProvider>
...
Transport bodies are extensible, but extension fields are application data,
not proof of identity or access. Resolve the authorized owner on the server
from the bearer token or secure session cookie and verify every thread,
channel, and run against it.
