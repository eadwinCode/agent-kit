...
To create shared, collaborative sessions, pass a server-authorized
`channelKey`. The key selects a real-time channel; it does not grant access to
one. Your token endpoint and HTTP handlers must independently authenticate the
caller and authorize that channel.

```tsx
import { AgentProvider } from "@inngest/use-agent";
import { CollaborativeChat } from "./CollaborativeChat";

function CollaborativeChatView({ channelGrant }) {
  return (
    <AgentProvider
      channelKey={channelGrant.channelKey}
      transport={{
        headers: { Authorization: `Bearer ${channelGrant.accessToken}` },
      }}
    >
      <CollaborativeChat />
    </AgentProvider>
