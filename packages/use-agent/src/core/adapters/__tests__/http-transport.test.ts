...
describe("DefaultHttpTransport - basics", () => {
  ...
  it("falls back to global fetch when not injected", async () => {
    ...
    const fetchSpy = vi.fn(async () =>
      jsonResponse({ success: true, threadId: "t2" })
    );
    ...
    try {
      ...
      const res = await transport.sendMessage(
        {
          userMessage: { id: "m1", content: "x", role: "user" },
          threadId: "t",
          history: [],
        },
        {}
...
describe("DefaultHttpTransport - URL building and params", () => {
  it("prefixes baseURL for relative endpoints", async () => {
    const fetchSpy = vi.fn(async () =>
      jsonResponse({ success: true, threadId: "t3" })
    );
    const transport = new DefaultHttpTransport({
      fetch: fetchSpy as unknown as typeof fetch,
    ...
    await transport.sendMessage({
      userMessage: { id: "m", content: "c", role: "user" },
      threadId: "th",
      history: [],
    });
  ...
  it("does not prefix baseURL for absolute endpoints", async () => {
    const fetchSpy = vi.fn(async () =>
      jsonResponse({ threads: [], hasMore: false, total: 0 })
    );
    const transport = new DefaultHttpTransport({
      fetch: fetchSpy as unknown as typeof fetch,
      baseURL: "https://base.invalid",
      api: { fetchThreads: "https://abs.example.com/threads" } as any,
    });
  ...
  it("fetches optional active-run replay envelopes", async () => {
    const events = [{ event: "run.started", sequenceNumber: 1 }];
    const fetchSpy = vi.fn(async () => jsonResponse({ events }));
    const transport = new DefaultHttpTransport({
      fetch: fetchSpy as unknown as typeof fetch,
      api: { fetchRunEvents: "/api/threads/{threadId}/run-events" } as any,
    });
    await expect(
      transport.fetchRunEvents({ threadId: "a b" })
    ).resolves.toEqual(events);
    const [url] = fetchSpy.mock.calls[0]! as unknown as FetchCall;
    expect(String(url)).toBe("/api/threads/a%20b/run-events");
  });
});
...
describe("DefaultHttpTransport - headers and body merging", () => {
  it("merges default headers with per-call headers (call overrides)", async () => {
    const fetchSpy = vi.fn(async () =>
      jsonResponse({ success: true, threadId: "t4" })
    );
    const transport = new DefaultHttpTransport({
      fetch: fetchSpy as unknown as typeof fetch,
      headers: async () => ({
...
describe("DefaultHttpTransport - response handling and errors", () => {
  ...
  it("throws with nested error message when present", async () => {
    ...
    await expect(transport.createThread({ title: "x" })).rejects.toMatchObject({
      message: "nested",
    });
  });
...
describe("DefaultHttpTransport - specific endpoints", () => {
  ...
  it("cancelMessage throws when endpoint not configured", async () => {
    ...
    const transport = new DefaultHttpTransport({
      fetch: fetchSpy as unknown as typeof fetch,
      api: { cancelMessage: undefined as any } as any,
    });
    await expect(transport.cancelMessage({ threadId: "t" })).rejects.toThrow(
      "cancelMessage endpoint not configured"
    );
  });

  it("passes AbortSignal through to fetch", async () => {
    const fetchSpy = vi.fn(async () =>
      jsonResponse({ success: true, threadId: "t7" })
    );
    const transport = new DefaultHttpTransport({
      fetch: fetchSpy as unknown as typeof fetch,
    });
